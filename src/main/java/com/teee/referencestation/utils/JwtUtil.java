...
public class JwtUtil {
    ...
    public static boolean verify(String token) {
        try {
            // 帐号加JWT私钥解密
            String secret = getClaim(token, Constant.ACCOUNT) + new String(Base64.getDecoder().decode(encryptJWTKey), "utf-8");
    ...
    public static String sign(String account, long userId, String currentTimeMillis) {
        try {
            // 帐号加JWT私钥加密
            String secret = account + new String(Base64.getDecoder().decode(encryptJWTKey), "utf-8");
            // 此处过期时间
            LocalDateTime localDateTime = LocalDateTime.now().plusSeconds(Long.parseLong(accessTokenExpireTime));
            Algorithm algorithm = Algorithm.HMAC256(secret);
            // 附带account帐号信息
            return JWT.create()
