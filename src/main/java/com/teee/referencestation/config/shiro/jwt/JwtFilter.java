package com.teee.referencestation.config.shiro.jwt;
...
public class JwtFilter extends BasicHttpAuthenticationFilter {
    ...
    private synchronized boolean refreshToken(ServletRequest request, ServletResponse response) {
        ...
        // 判断Redis中RefreshTokenBackList是否存在
        if (RedisUtil.exists(Md5EncryptionUtil.md5WithNoSalt(token))) {
            String newToken = RedisUtil.getObject(Md5EncryptionUtil.md5WithNoSalt(token)).toString();
        ...
        if (RedisUtil.exists(Constant.PREFIX_SHIRO_REFRESH_TOKEN + account)) {
            ...
            String curr = JwtUtil.getClaim(token, Constant.CURRENT_TIME_MILLIS);
            if (curr.equals(currentTimeMillisRedis)) {
                // 获取当前最新时间戳
                ...
                String newToken = JwtUtil.sign(account, userId, currentTimeMillis);
                // 将新刷新的AccessToken再次进行Shiro的登录
                JwtToken jwtToken = new JwtToken(newToken);
                // 提交给UserRealm进行认证，如果错误他会抛出异常并被捕获，如果没有抛出异常则代表登入成功，返回true
                this.getSubject(request, response).login(jwtToken);
                ...
                httpServletResponse.setHeader("Authorization", newToken);
                httpServletResponse.setHeader("Access-Control-Expose-Headers", "Authorization");
                // 防止并发请求，例如一个页面同时发起多个请求，这几个请求都携带了过期token，导致第一个请求刷新后，其它请求都失效
                String refreshTokenExpireBackListTime = PropertiesUtil.getProperty("refreshTokenExpireBackListTime");
                RedisUtil.setObject(Md5EncryptionUtil.md5WithNoSalt(token), newToken, Long.valueOf(refreshTokenExpireBackListTime));
                return true;
            }
        }
    ...
    private void response401(ServletRequest req, ServletResponse resp, String msg) {
        ...
        try(PrintWriter out = httpServletResponse.getWriter()) {
            out.append(JsonUtil.toJSONString(new RestResponse<>(HttpStatus.FORBIDDEN.value(), "无权访问(Unauthorized):" + msg)));
        } catch (IOException e) {
